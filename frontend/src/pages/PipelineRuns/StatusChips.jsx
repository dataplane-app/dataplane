...
export default function StatusChips() {
    ...
    return (
        <Grid item display="flex" alignItems="center" sx={{ alignSelf: 'center' }} mr={1}>
            ...
            />
            <CustomChip
                amount={(RunState.runObject?.nodes?.get() && Object?.values(RunState.runObject?.nodes.get())?.filter((a) => a?.status === 'Run').length) || '0'}
                label="Running"
                margin={2}
                customColor="blue"
            />
            <CustomChip
                amount={(RunState.runObject?.nodes?.get() && Object?.values(RunState.runObject?.nodes.get())?.filter((a) => a?.status === 'Success').length) || '0'}
                label="Succeeded"
                margin={2}
                customColor="green"
            />
            <CustomChip
                amount={(RunState.runObject?.nodes?.get() && Object?.values(RunState.runObject?.nodes.get())?.filter((a) => a?.status === 'Fail').length) || '0'}
                label="Failed"
                margin={2}
                customColor="red"
            />
